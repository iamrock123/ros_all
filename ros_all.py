...
class MinimalSubscriber(Node):
    ...
    def __init__(self):
        ...
        self.publisher_ = self.create_publisher(
            msg.Image, 'cvImage', 10)  # 加入 ROS-publisher, 發出處理過的image
        self.publisher_facename = self.create_publisher(String, 'facename', 10)

        self.subscription  # prevent unused variable warning
        self.telloCli = self.create_client(
            TelloAction, 'tello_action')  # TODO: 待完成，call ros2 service
        ...
        for item in self.names:  # ! 讀取在 dataset_img 下的全部圖片
            l['%s_image' % item] = face_recognition.load_image_file(
                "dataset_img/%s.jpg" % item)
            if len(face_recognition.face_encodings(l['%s_image' % item])) != 0:
                l['%s_face_encoding' % item] = face_recognition.face_encodings(
                    l['%s_image' % item])[0]
    ...
    def listener_callback(self, image):  # ! 從image讀到cam image後觸發的 function
        ...
        small_frame = cv2.resize(
            cv_image, (0, 0), fx=0.5, fy=0.5)  # resize frame
        ...
        #     self.sendRequest(instruction)
            ...
            self.aruLockCode = ''
            self.aruLock = False
            aru_distance[index] = aru_distance[index] *2
            ...
            for ids in aru_id:
                ...
                if aru_distance[index] < self.aruBound[0]:
                    self.sendRequest("rc 0 -10 0 0")  # x z ys
                    self.aruLockCode = 'back'
                    self.aruLock = True
                    continue
                if aru_distance[index] > self.aruBound[2]:
                    if flag == 0:
                        self.sendRequest("cw 360")
                        flag = 1
                        print("flag:",flag)
                    if aru_x[index] < -0.01:#-0.22
                        instruction[1] = '-15'#-11
                        instruction[2] = '0'
                ...
                #elif aru_distance[index] < self.aruBound[1]:
                   # instruction[2] = '10'
                elif not align:
                    print("z aligned")
                    if aru_x[index] < -0.06:
                    ...
                    if aru_y[index] < -0.33:#-0.33
                        instruction[3] = '20'#13
                        if self.locationFixed:

                            self.aruLocation['bottom'] = True
                            self.aruLocation['top'] = False
                    elif aru_y[index] > -0.01:#-0.01
                        instruction[3] = '-20'#-13
                        if self.locationFixed:
                    ...
                    if x and y:
                        ...
                        ins = ' '.join(instruction)
                        self.sendRequest(ins)
                # print(ids[0], aru_distance[index])#and self.aruTarget == 1:
                if align:
                    if ids[0] == 5:
                        if aru_distance[index] < self.aruBound[1]:
                            self.sendRequest("cw 270")
                            flag = 0
                    elif ids[0] == 6:
                        if aru_distance[index] < self.aruBound[1]:
                            self.sendRequest("cw 270")
        ...
        if self.stop == False and self.mode == 'normal':
                    ...
                    for num in range(0, 1, 1):
                        if self.dark == True:
                            matches = face_recognition.compare_faces(
                                self.known_face_encodings, face_encoding, tolerance=0.9)
                        else:
                            matches = face_recognition.compare_faces(
            ...
            for (top, right, bottom, left), name in zip(self.face_locations, self.face_names):
                ...
                if self.unknowTakeAgain and name == self.unknownTakeAgainName and self.unknownTakeAgainCount > 3:
                    ...
                    cv2.imwrite('./dataset_img/%s_%s.jpg' %
                                (self.unknownTakeAgainName, randstr), i)
                    self.unknowTakeAgain = False
                    self.unknownTakeAgainName = ''
                    self.unknownTakeAgainCount = 0
                execute = ['rc', '0', '0', '0', '0']
                if namePut[0] == self.followName:
                    ...
                    if self.xulyframe >= 1:
                        ...
                        if xalign and yalign and distanceAlign:
                            cv2.putText(cv_image, "aligned", (left + 20, bottom + 20),
                ...
                cv2.rectangle(cv_image, (left, top),
                              (right, bottom), (0, 0, 255), 2)

                # Draw a label with a name below the face
                cv2.rectangle(cv_image, (left, bottom - 35),
                              (right, bottom), (0, 0, 255), cv2.FILLED)
                font = cv2.FONT_HERSHEY_DUPLEX
                cv2.putText(cv_image, namePut[0], (left + 6, bottom - 6),
                            cv2.FONT_HERSHEY_SIMPLEX, 1.0, (255, 255, 255), 1)
            ...
            for (box, pred) in zip(locs, preds):
                ...
                cv2.rectangle(cv_image, (startX, startY),
                              (endX, endY), color, 2)
